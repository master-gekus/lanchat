...
namespace
{
  extern quint32 crctable[256];
}

quint32 MessageComposer::crc32(quint32 crc, const void* data, size_t data_len)
{
  crc ^= 0xffffffffL;
...
QByteArray MessageComposer::composeNonEncrypted(const GJson& msg)
{
  ...
  QByteArray message;
  if ((compressed.size() - 4) < uncompressed.size())
    {
      message.resize(compressed.size() + 2);
      char *data = message.data();
      *((quint16*)(data)) = (uncompressed.size() & 0x3FFF) | 0x8000;
      memmove(data + 2, compressed.constData() + 4, compressed.size() - 4);
    }
  else
    {
      message.resize(uncompressed.size() + 6);
      char *data = message.data();
      *((quint16*)(data)) = 0;
      memmove(data + 2, uncompressed.constData(), uncompressed.size());
    }

  *((quint32*)(message.data() + message.size() - sizeof(quint32)))
